...
/**
 * 데모용 Mock 함수
 * 실제 Veo API 사용 전까지 테스트용으로 사용
 */
export async function generateVideoWithVeoMock(
  prompt: string
): Promise<VeoVideoResponse> {
  console.log("🎬 Mock 비디오 생성:", prompt);
...
/**
 * Veo API로 생성된 비디오를 서버에서 다운로드하여 Base64로 반환
 * 클라이언트에서 직접 다운로드하면 403 에러 발생 (API 키 필요)
 */
export async function downloadVeoVideoAsBase64(videoUrl: string): Promise<{
  success: boolean;
  error?: string;
  videoBase64?: string;
  videoBlob?: string;
  ...
  try {
    ...
    console.log("✅ 비디오 다운로드 완료 (Base64 변환됨)");
    console.log(
      `📊 파일 크기: ${(arrayBuffer.byteLength / 1024 / 1024).toFixed(2)} MB`
    );

    return {
      success: true,
      videoBase64, // data URL 형식
    };
  } catch (error) {
