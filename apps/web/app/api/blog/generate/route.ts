...
- **목표 글 길이:** **공백을 제외한 순수 텍스트 기준 ${details.length} 내외** (공백 포함 시 약 ${Math.round(parseInt(details.length) * 1.3)} ~ ${Math.round(parseInt(details.length) * 1.5)}자 사이가 적당합니다.)
...
1. **글 길이 엄수 (CRITICAL):**
   - **HTML 태그, CSS 코드, 그리고 '공백(Space/Newline)'을 모두 제외한 순수 한글/영문/숫자 텍스트**가 반드시 **${details.length}**에 최대한 가깝게(오차 범위 10% 이내) 작성되어야 합니다.
   - 분량을 채우기 위해 의미 없는 말을 반복하거나 서술을 길게 늘어뜨리지 마세요. 
   - 만약 내용이 부족하다면 새로운 관점이나 구체적인 사례를 추가하고, 내용이 너무 많다면 핵심 위주로 문장을 정제하여 지정된 길이를 맞추세요.
   - 문단 사이의 적절한 공백과 줄바꿈은 가독성을 위해 반드시 필요하지만, 이를 분량 계산에 포함하지 마세요.
