            ...
            async start(controller) {
                try {
                            ...
                            if (matches.length > 0) {
                                ...
                                const imagePromises = matches.map(async (match) => {
                                    ...
                                    if (imageResult.success && imageResult.imageUrl) {
                                        ...
                                        const matches = imageResult.imageUrl.match(/^data:(.+);base64,(.+)$/);
                    ...
                    if (formData.gif?.youtubeUrl && formData.gif?.startTimes?.length > 0) {
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: 4, message: '유튜브 GIF 생성 중...' })}\n\n`));
                        ...
                        try {
                            ...
                            if (lambdaUrl) {
                                ...
                                if (response.ok) {
                                    ...
                                    console.error('Lambda GIF generation failed:', await response.text());
                                }
                            } else {
                        ...
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: 4, message: '유튜브 GIF 선택 안함' })}\n\n`));
                        await new Promise(resolve => setTimeout(resolve, 500));
                    }

                    // Step 5: 원장님 사진 처리
                    if (formData.photo?.originalUrl) {
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: 5, message: '원장님 사진 보정 중...' })}\n\n`));
                        ...
                        try {
                            ...
                            if (textContent.includes(placeholder)) {
                                ...
                                const instruction = "Professional doctor portrait, trustworthy atmosphere, high quality, natural lighting. Maintain the person's identity but enhance the professional look suitable for a medical blog.";
                                ...
                                if (editedFile && 'base64Data' in editedFile && editedFile.base64Data) {
                                    ...
                                    console.warn('Director photo editing failed or returned no data');
                                }
                            } else {
                                console.log('Director photo provided but no placeholder found in text.');
                            }
                        ...
                        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', step: 5, message: '원장님 사진 건너뜀' })}\n\n`));
                        await new Promise(resolve => setTimeout(resolve, 500));
                    }
