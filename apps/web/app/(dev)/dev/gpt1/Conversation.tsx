...
export default function Conversation({ sessionId }: { sessionId: string }) {
  ...
  async function createThumbnail() {
    ...
    try {
      if (!datas.selectedThumbnailGuideIndex)
        toast.error("썸네일 가이드를 선택해주세요.");
      const selectedTitleSet =
        datas.titleResponses?.sets[datas.selectedTitleIndex];
      const thumbnailGuide =
        datas.thumbnailGuideResponses?.thumbnailGuides[
          datas.selectedThumbnailGuideIndex
        ];
  ...
  return (
    <div className="flex flex-col gap-4 mt-12 p-4 border rounded-lg">
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          {isTitleLoading ? (
            <LoadingSpinner loadingText="제목 생성 중..." />
          ) : (
            "제목 생성"
          )}
        </Button>
      ...
      {datas.titleResponses && (
        <div className="flex flex-col gap-2">
          <div className="grid grid-cols-3 gap-2">
            {datas.thumbnailGuideResponses?.thumbnailGuides?.map(
                  ...
                  >
                    <CardHeader>
                      <CardTitle className="flex flex-wrap">
                        <MarkdownRenderer
                          content={`**${index + 1}**. ${guide.guideTitle}`}
                        />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                    ...
                    <CardFooter>
                      <Button onClick={() => selectThumbnailGuide(index)}>
                        선택
                      </Button>
                    </CardFooter>
                  </Card>
        ...
        <div className="flex flex-col gap-2">
          <Button
            ...
            disabled={
              isThumbnailLoading ||
              step !== "THUMBNAIL" ||
              !datas.selectedThumbnailGuideIndex === null
            }
          >
            {isThumbnailLoading ? (
              <LoadingSpinner loadingText="썸네일 생성 중..." />
            ) : (
              "썸네일 생성"
            )}
          </Button>
          {datas.thumbnailResponses && (
            <div className="grid grid-cols-2 gap-4">
              <div className="border rounded-lg bg-white overflow-hidden">
                <img
                  src={`data:image/jpeg;base64,${datas.thumbnailResponses}`}
                  alt="썸네일"
                />
              </div>
              <Card>
      ...
      {datas.thumbnailResponses && (
        <div className="flex flex-col gap-2">
          <Button
            ...
            disabled={isScriptLoading || step !== "SCRIPT"}
          >
            {isScriptLoading ? (
